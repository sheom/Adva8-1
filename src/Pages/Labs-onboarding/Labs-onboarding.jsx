...
function LabsOnboarding() {
  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + labDetails.imageUrls.length < 7) {
      setUploading(true);
      ...
      Promise.all(promises)
        .then((urls) => {
          ...
          // setFormData({
          //   ...formData,
          //   imageUrls: labDetails.imageUrls.concat(urls),
          // });
          setLabDetails({ 
            ...labDetails, 
            imageUrls: labDetails.imageUrls.concat(urls),
            //labImages: e.target.files[0] 
          });
  ...
  const handleRemoveImage = (index) => {
    setLabDetails({ 
      ...labDetails, 
      imageUrls: labDetails.imageUrls.filter((_, i) => i !== index),
    });
  ...
  const handleSubmit = (e) => {
    ...
    alert("Check Payload At console");
    console.log("Payload after alert: " + JSON.stringify(payload));
    ...
    fetch("/api/lab/create", {
      ...
      body: JSON.stringify(payload.labDetails),
    })
          ...
          <div className="form-row">
            ...
            <div className="column">
              ...
              <div className="flex flex-col flex-1 gap-4">
                ...
                {labDetails.imageUrls.length > 0 &&
                  labDetails.imageUrls.map((url, index) => (
                    <div
                      key={url}
